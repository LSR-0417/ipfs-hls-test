...
- 若開發過程導致需求、設計或驗證條件變動，應同步更新對應 Issue，並與團隊協作更新 SRS、SDDD，讓需求文件與實作保持一致。
- RD 應依 SDDD 與 PM 提供的 `User Story`，以及 TE 提供的 `Spec. Test`、`User Scenario Test`、`Edge Test` 實作功能。
- RD 可以針對這些測試情境進行實作邊界限定，例如明確定義 `Input Box` 可接受的型別、上下限範圍，或標注某些情境不實作。
- 若某些情境不實作，驗證通過標準應改為是否有正確防禦、正確阻擋，或正確回應預期錯誤，而不是假裝完整支援。
- 若 `User Scenario Test` 驗出錯誤，應先回報 RD 檢查 SDDD 是否需要修正，再同步調整實作、Issue、User Story 與測試內容。
...
- 不要在需求或設計已經改變後，只改 code 而不更新 Issue、SRS、SDDD。
- 不要在 `User Scenario Test` 已經顯示設計不一致時，只 patch code 而不回頭更新 SDDD 與相關文件。
