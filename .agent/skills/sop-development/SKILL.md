...
7. If requirements, design, or validation conditions change during implementation, update the corresponding issue and work with the team to sync `SRS` and `SDDD` before continuing.
8. Implement according to `SDDD`, the PM-provided `User Story`, and the TE-provided `Spec. Test`, `User Scenario Test`, and `Edge Test`.
...
11. If `User Scenario Test` exposes a mismatch, report it back, check whether `SDDD` needs correction, and then sync implementation, issue, `User Story`, and test content.
...
- Do not patch around `User Scenario Test` mismatches without checking whether the design or validation baseline must change.
...
- The current `User Story`, `Spec. Test`, `User Scenario Test`, or `Edge Test` baseline has changed and implementation direction is no longer clear.
