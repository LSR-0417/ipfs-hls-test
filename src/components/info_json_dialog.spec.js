...
describe('InfoJsonDialog tabbed draft contract', () => {
  it('separates metadata, subtitles, and video processing into independent tabs', () => {
    ...
    expect(script).not.toContain('const effectiveIncludeInfoJson = computed(');
    expect(script).not.toContain('const effectiveIncludeSubtitleManifest = computed(');
    expect(script).not.toContain('const processorAttachmentCount = computed(');
    expect(script).not.toContain('const videoProcessorDraftJson = computed(');
    expect(script).not.toContain('function buildLocalVideoProcessorDraft(file, options = {}) {');
    ...
    expect(template).toContain("t('infoJson.form.caption')");
    expect(template).toContain("t('infoJson.assets.subtitles.caption')");
    expect(template).toContain("t('infoJson.assets.video.caption')");
    expect(template).toContain('class="panel-actions panel-actions--end"');
    expect(template).toContain('class="panel-head panel-head--responsive"');
    expect(template).toContain('class="panel-block"');
    expect(template).toContain('class="meta-chip"');
    ...
    expect(template).not.toContain("t('infoJson.panels.metadata.title')");
    expect(template).not.toContain("t('infoJson.panels.subtitles.title')");
    expect(template).not.toContain("t('infoJson.panels.video.title')");
    expect(template).not.toContain("t('infoJson.video.draftBadge')");
    expect(template).not.toContain('@click="copyInfoJson"');
    expect(template).not.toContain('class="panel-surface');
    ...
    expect(style).toContain('.panel-head');
    expect(style).toContain('.panel-inline-meta');
    expect(style).toContain('.meta-chip');
    expect(style).toContain('.panel-block');
    ...
    expect(style).not.toContain('.panel-surface');
    expect(style).not.toContain('.preview-card');
    expect(style).not.toContain('.json-preview');
    expect(style).not.toContain('.asset-meta-banner');
