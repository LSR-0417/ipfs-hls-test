...
一個基於 Vue 3 + Vite 的 IPFS 多網關 HLS 視頻播放器。支援多質量自適應流媒體播放、字幕切換、時間分享、觀看歷史與 sidecar metadata 載入。
...
3. **載入影片** - 點擊「▶️ 載入影片」按鈕
4. **查看歷史** - 從側邊欄切到 `History`，查看最近播放內容與續播狀態
5. **分享片段** - 點擊「🔗 分享片段」複製包含目前播放時間的連結
...
觀看歷史會保存在瀏覽器的 `localStorage`，目前只記錄在本機，不會跨裝置同步。

目前會記住：

- 最近看過的 `CID`
- 影片標題、上傳者、封面與時長（若 sidecar metadata 可用）
- 最後播放進度
- 最近觀看時間
- 當時使用的 gateway
