...
- 全域鍵盤快捷鍵：`Space / K` 播放控制、左右鍵 `±5` 秒、`J / L` `±10` 秒、`M` 靜音、`F` 全螢幕、`C` 字幕、`,` / `.` 逐幀、`?` 快捷鍵說明
...
- 按下 `C` 會在字幕關閉與「上次使用的字幕語言」之間切換
- 切換結果需同步更新字幕偏好儲存

### FR-KEY-9 逐幀播放快捷鍵

當播放器為暫停狀態且頁面接收到鍵盤事件時：

- 按下 `,` 會往前逐幀
- 按下 `.` 會往後逐幀
- 單次位移秒數優先取自 `info.json.fps`，若無有效 `fps` 則退回預設 frame rate
...
- 按下 `?` 會開啟或關閉快捷鍵說明對話框
- 對話框開啟時，播放快捷鍵不應再控制播放器
- 對話框開啟時，按下 `Escape` 會關閉對話框
...
- 當播放器為播放狀態且焦點不在互動控制項內，按下 `K` 後播放器暫停播放
- 當焦點不在互動控制項內時，按下 `J` / `L` 會分別倒退 / 前進 `10` 秒
- 當焦點不在互動控制項內時，按下 `M` 會切換靜音狀態
- 當焦點不在互動控制項內時，按下 `F` 會切換播放器全螢幕狀態
- 當存在字幕軌時，按下 `C` 會在字幕關閉與上次使用語言之間切換
- 當播放器已暫停且 `fps` 為 `20` 時，按下 `.` 後播放時間會前進 `0.05` 秒
- 按下 `?` 後會顯示快捷鍵說明對話框；對話框開啟時按下 `Escape` 會關閉
- 當播放器為播放狀態且焦點不在互動控制項內，按下空白鍵後播放器暫停播放
- 當焦點在搜尋輸入框或其他可編輯區域時，左右鍵、空白鍵與其他播放快捷鍵不得接手播放器控制
