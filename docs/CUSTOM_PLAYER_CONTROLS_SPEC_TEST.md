...
- 控制列顯示策略固定為：
  ...
  - 滑鼠離開播放器區域時立即隱藏
  - 單純閒置時也可進入隱藏
  - fullscreen 下若控制列因閒置而隱藏，桌機與可 hover 裝置的滑鼠游標也需同步隱藏
...
驗證一般模式與 fullscreen 的控制列顯示策略一致，且 fullscreen 閒置時的滑鼠游標行為符合契約。
...
5. fullscreen 下若控制列因閒置而隱藏，桌機與可 hover 裝置的滑鼠游標也會同步隱藏；移動滑鼠或打開互動面板後會恢復顯示。
