...
- [`DUAL_SUBTITLE_SPEC_TEST.md`](./DUAL_SUBTITLE_SPEC_TEST.md)
...
- 播放器控制列上的主字幕入口提供主字幕快速切換。
- 播放器控制列需另外提供一個獨立的字幕總開關。
- `SubtitleDialog` 提供完整的主字幕 / 次字幕配置與字幕資源管理。
- `SubtitleDialog` 內的主字幕 / 次字幕配置以單一字幕清單呈現，並以按鈕操作搭配 row outline 標示目前狀態。
- 主字幕與次字幕不得選擇同一語言。
- 畫面上的字幕渲染順序預設為：
  - 主字幕在下
  - 次字幕在上
...
- `mode = 'showing'` 時，允許：
  - 只有主字幕
  - 主字幕加次字幕
- 首次載入且沒有既有持久化偏好時，只要存在可用字幕，系統應自動進入 `mode = 'showing'`
- 首次自動選擇主字幕時，應先比對瀏覽器語系；若無匹配則 fallback 到英文，再 fallback 到第一條可用字幕
...
### FR-SUB-5 `SubtitleDialog` 主 / 次字幕配置入口

`SubtitleDialog` 負責主字幕 / 次字幕配置與字幕資源管理。

`SubtitleDialog` 必須提供：

- 單一字幕清單
- 每條字幕列的 `設為主字幕` / `設為次字幕` 操作
- 主字幕 / 次字幕 row outline 狀態標示
- 匯入字幕
- 移除本機字幕
- 下載字幕
...
同理，若使用者在主字幕選單或 `SubtitleDialog` 內把主字幕改成目前次字幕語言，系統必須避免主次字幕重複。
...
- 匯入流程可一次選擇多個 `.srt` / `.vtt`
- 每一條字幕需獨立完成轉檔、排序與結果回饋
- 必須立即進入 `availableTracks`
- 必須立即出現在主字幕選單與 `SubtitleDialog` 的字幕清單中
...
播放器控制列需提供一個獨立的字幕總開關 action button。
...
字幕總開關 action button 與 `C` 鍵都不負責切換主字幕語言，也不負責切換次字幕語言。
...
5. `SubtitleDialog` 內以單一字幕清單提供主字幕 / 次字幕切換，且每條字幕列有清楚的主 / 次狀態外框。
6. 匯入字幕後，主字幕選單與 `SubtitleDialog` 清單都能立即看到新字幕。
7. `SubtitleDialog` 內可直接把任一字幕設為主字幕或次字幕，且不會讓主次字幕重複。
8. 只開主字幕時，不顯示 `A/B` 按鈕。
9. 同時開主字幕與次字幕時，`A/B` 按鈕可正常交換上下角色。
10. 次字幕尺寸小於主字幕，且位置貼齊主字幕上方，不出現過大固定間距。
11. `subtitles.json` 載入慢時，使用者能看到 loading 狀態，而不是誤判成沒有字幕。
12. `.vtt` 下載較慢時，主字幕按鈕仍可操作，實際字幕可稍後顯示。
13. 控制列存在獨立字幕總開關 action button，點擊或按 `C` 只切換總開關，不改變目前主字幕 / 次字幕語言組合。
14. 首次載入且沒有既有偏好時，字幕會自動開啟；若瀏覽器語系沒有匹配，則 fallback 到英文，再 fallback 到第一條可用字幕。
